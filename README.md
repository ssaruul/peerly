...
The server keeps, per world: the latest session save of each of the 5 most recent hosts; the last save of each of the last 3 days and of the last 2 weeks that had sessions; 3 mid-session backups; and every branch save younger than 30 days (plus the 30 newest older ones). A save stays as long as any one of these reasons applies, and the reasons overlap heavily, so a group of four ends up with about 6 to 8 files per world, 2 GB for a 300 MB world. One person's sessions replace each other, so however often one friend hosts, the last save of everyone before them stays, and the daily and weekly saves mean a problem noticed a week later can still be undone. History still lists every session for six months after its file was removed, so the group can always see who hosted when. Any save can be marked **Keep** in History; a kept save is never removed automatically, which is the thing to do after a good evening or before a risky game update. A PC whose last synced copy is no longer on the server uploads that copy as a branch before replacing it.
